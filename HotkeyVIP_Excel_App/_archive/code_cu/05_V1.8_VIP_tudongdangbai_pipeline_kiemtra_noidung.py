...
def find_next_word_file_from_excel() -> tuple[int, str, Path]:
    ...
    - Ưu tiên dòng có Trạng thái chứa "cần mở", bằng "Ok"
      hoặc chứa "lỗi kiểm tra".
    ...
    # Nhóm cần xử lý được chọn theo thứ tự dòng Excel.
    row = 2
    while True:
    ...
    # Nếu không có dòng cần xử lý, dùng dòng trạng thái trống.
    ...
    if target_row is None:
        raise RuntimeError(
            'Không còn bài nào có Trạng thái "cần mở", "Ok", '
            '"LỖI KIỂM TRA" hoặc đang trống.'
        )
...
def write_publish_result(
    ...
    sheet.Cells(row, find_column_by_header(sheet, PUBLISH_HEADERS["published_at"])).Value = datetime.now()
    ...
    try:
        ...
        try:
            cms_id, edit_url, public_url = find_saved_article_info(
                driver,
                # Chi doi chieu theo tieu de hien thi (H1). Ten file Word va
                # Title SEO co the trung voi hang khac trong danh sach CMS.
                [data["name"]],
            )
...
def main() -> int:
    ...
    try:
        ...
        with ThreadPoolExecutor(max_workers=1, thread_name_prefix="WordPrefetch") as prefetch:
            ...
            for article_index in range(1, total_articles + 1):
                ...
                try:
                    ...
                    # process_one_article() đã ghi trạng thái và Note vào Excel.
                    # Lỗi đăng chỉ bỏ qua dòng này, không dừng toàn bộ pipeline.
                    publish_errors.append(
                        {"article_index": article_index, "error": str(exc)}
                    )
                    progress_window.update(
                        article_index,
                        f"Skipped article {article_index}: publish failed; continuing",
                    )
                    print(
                        f"[SKIP {article_index}] Publish failed; "
                        "Excel đã ghi LỖI ĐĂNG, tiếp tục bài kế tiếp."
                    )
                    continue
        ...
        if publish_errors:
            print(
                f"- Lỗi đăng {len(publish_errors)} bài; đã ghi vào Excel "
                "và các bài còn lại vẫn tiếp tục."
            )
        if not validation_skips and not publish_errors:
