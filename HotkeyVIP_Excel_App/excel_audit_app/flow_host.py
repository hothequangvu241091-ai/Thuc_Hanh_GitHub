...
def _workbook_run_lock_path(workbook_path: Path) -> Path:
    """Trả về lock riêng cho đúng workbook, không phụ thuộc tên file."""
    normalized = os.path.normcase(os.path.abspath(str(workbook_path)))
    key = hashlib.sha256(normalized.encode("utf-8")).hexdigest()[:20]
    runtime = _owned_excel_registry_path().parent
    return runtime / f"flow_{key}.lock"
...
def _acquire_workbook_run_lock(workbook_path: Path) -> Path:
    """Không cho hai flow cùng thao tác một workbook trong hai Excel instance."""
    lock_path = _workbook_run_lock_path(workbook_path)
    payload = {
        "pid": os.getpid(),
    ...
    for _attempt in range(2):
        try:
            ...
            if _pid_is_running(existing.get("pid")):
                raise RuntimeError(
                    "Đang có một flow khác thao tác workbook này. "
                    "Hãy chờ flow đó kết thúc hoặc dừng an toàn trước khi chạy flow mới."
                )
            try:
                lock_path.unlink()
            except OSError as exc:
                raise RuntimeError(
                    "Không thể dọn lock của flow cũ; hãy đóng app rồi thử lại."
                ) from exc
    raise RuntimeError("Không thể tạo lock bảo vệ workbook.")


def _release_workbook_run_lock(lock_path: Path | None) -> None:
...
def run_flow(workbook_path: Path, script_path: Path, script_args: list[str]) -> int:
    ...
    run_lock: Path | None = None
    try:
        run_lock = _acquire_workbook_run_lock(workbook_path)
        # Một flow trước có thể bị tắt cưỡng bức khi Excel đang mở hộp thoại.
        # Khi đó chỉ dọn các Excel ẩn đã được app ghi nhận là của chính nó.
        cleaned = _cleanup_tracked_hidden_excel()
        if cleaned:
            print(
                f"[APP] Đã dọn {cleaned} Excel ẩn còn sót từ phiên flow trước.",
                flush=True,
            )
        ...
        _release(excel)
        _release_workbook_run_lock(run_lock)
