...
VERSION = "V2.2_dieu_phoi_bai_viet_lien_quan_solo"
...
def choose_worker_count() -> int:
    if len(sys.argv) >= 2:
        ...
        raw_value = input("Chọn số luồng [3/5] (nhấn Enter = 3): ").strip()

    if not raw_value:
        return 3
    if raw_value not in {"3", "5"}:
        raise SystemExit("Chỉ được chọn 3 hoặc 5 luồng.")
